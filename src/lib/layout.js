...
export function get() {
  let offset = 8;
  const count = metadata.readUInt32LE(0);
  const base = metadata.readUInt32LE(4);
  const layout = { };
  for (let i = 0; i < count; ++i) {
    const length = metadata.readUInt32LE(offset + 0);
    const port = metadata.readUInt32LE(offset + 4);
    const mask = metadata.readUInt32LE(offset + 8);
    const start = metadata.readUInt32LE(offset + 12);
...
export function set(layout) {
  ...
  metadata.writeUInt32LE(keys.length, 0);
  metadata.writeUInt32LE(pru.l3.address, 4);
  keys.forEach(pinId => {
    ...
    pin.direction = 'out';
    metadata.writeUInt32LE(length, offset + 0);
